...
object HourlyTipsExercise {
  ...
  def main(args: Array[String]) {
    ...
    val hourlyTips = fares.map((f: TaxiFare) => (f.driverId, f.tip)).keyBy(f => f._1).timeWindow(Time.hours(1)).reduce((f_1: (Long, Float), f_2: (Long, Float)) => (f_1._1, f_1._2 + f_2._2), new WrapWithWindowInfo())
